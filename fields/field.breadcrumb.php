<?php
	...
	class FieldBreadcrumb extends Field {
		...
		public function buildDSRetrivalSQL($data, &$joins, &$where, $andOperation = false) {
			...
			if (preg_match('/^all-children-of:\s*(.*)/', $data[0], $matches)) {
				$mode = 'all-children-of';
				$paths = preg_split('%/%', $matches[1], 0, PREG_SPLIT_NO_EMPTY);
			}

			else if (preg_match('/^children-of:\s*(.*)/', $data[0], $matches)) {
				$mode = 'children-of';
				$paths = preg_split('%/%', $matches[1], 0, PREG_SPLIT_NO_EMPTY);
			}
			...
			if ($mode == 'all-children-of') {
				$field = $this; $entry_ids = array();
				$recursor = function($entry_id) use (&$entry_ids, $field, &$recursor) {
					foreach ($field->driver->getBreadcrumbChildren($field, $entry_id) as $child) {
						$entry_ids[] = $child->entry;
						$recursor($child->entry);
					}
				};
				...
				//var_dump($entry_id, $entry_ids); exit;
				...
				$where .= sprintf("
					AND (
						t{$field_id}_{$this->_key}.entry_id IN (%s)
					)",
					implode(', ', $entry_ids)
				);
			}
